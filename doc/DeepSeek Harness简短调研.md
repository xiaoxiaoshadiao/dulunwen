...
**第一，人家是真在为缓存做设计，不是顺带提一嘴。** 工具按名字排序、Compaction专门去复用前缀、Skill目录宁可追加一份新的也不改旧的、运行时上下文放历史末尾而不是塞进System Prompt——这四件事看着不挨着，背后其实是同一个判断：**改动的位置越靠前越贵。** 这句话在它的设计笔记里反复出现。对我们的直接影响是，做能力检索光盯着"找得准不准"不够，还得算一笔"这回换不换划算"。
