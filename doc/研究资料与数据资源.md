...
> 本文只记录智能体能力检索与运行时组合方向的论文、数据、代码和生态入口。论文PDF已归档；数据集、模型权重、插件包和容器镜像暂未下载。主线讨论见《智能体能力的发现、组织与执行》，方法与结果梳理见《相关论文逐篇解读》，DeepSeek内容见《DeepSeek Harness简短调研》和《DeepSeek Harness完整调研》。

当前远程论文目录为 `../papers/`，`SHA256SUMS` 记录已下载PDF的校验值。资源规模来自论文或官方页面，后续正式使用前仍需复核版本、许可、下载状态和数据字段。

## 一、相关论文归档

论文PDF下载到远程目录 `../papers/`。本表给出官方来源和本地文件名。
...
|材料|来源|主要内容|当前用途|
|---|---|---|---|
|从模型到Harness：WorkBuddy如何把Agent做成可用产品|WorkBuddy团队公众号文章，公开链接待补|Context Engineering、渐进式能力加载、Memory、Harness、Loop和产品验证|理解真实产品团队关注的问题，校准研究范围|
|DeepSeek Harness源码|`../deepseek-harness/`|插件架构、Agent Loop、Prompt、Tools、Session、Skill和Sub-Agent|验证产品原则在开源Harness中的具体实现|
|Koishi插件生态|Koishi Registry与Cordis案例|真实插件安装、依赖和运行时组合|观察Plugin作为产品分发和运行单元的边界|

WorkBuddy文章已经说明能力发现、按需展开、Prompt Cache和Harness控制是真实产品问题，但没有证明运行时自动Plugin Retrieval是必要形态。后续需要通过产品访谈和真实Trace确认检索发生在哪一层、主要失败是什么，以及可用指标和数据有哪些。
