...
> 能力检索与运行时组合方向的论文、数据、代码和生态入口。**17篇论文PDF已归档，数据集、模型权重、插件包和容器镜像都还没下载。** 表中规模数字取自论文或官方页面，正式使用前需要复核版本、许可和字段。

## 一、相关论文归档

`../papers/`下17个PDF加一份`SHA256SUMS`。
...
| DeepSeek Harness | 237 个 workspace 包（`packages/*/*` 219、vendor 9）；基础 bundle 挂 78 个插件 | https://github.com/deepseek-ai/deepseek-harness | 最贴近 Harness Plugin，但没有开放插件目录 |
...
WorkBuddy文章说明能力发现、按需展开、Prompt Cache和Harness控制是真实产品问题，但没有证明运行时自动Plugin Retrieval是必要形态。这一步要靠产品访谈和真实Trace来定：检索发生在哪一层、主要失败是什么、有哪些指标和数据可取。
